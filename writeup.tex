...
\emph{iterated logarithmic multiplication problem} (ILMP). Let $n \in \N$, $x_1, \ldots, x_n \in
[q-1]$, and $y_1, \ldots y_n \in [q-1]$. Let $X_i = g^{x_i}$ and $Y_i = g^{y_i}$
for each $i \in [n]$. Given the sequences $(X_1, \dots X_n)$ and $(Y_1, \dots,
Y_n)$, the problem is to decide if $g^{x_1 \cdots x_n} = g^{y_1 \cdots y_n}$.
The following is an interactive proof for the ILMP.
\begin{figure}[h]
  \oneCol{0.80}{
  \underline{$\brackets{P,V}\big( (X_1, \cdots, X_n), (Y_1, \dots,  Y_n)\big)$}:
  \begin{enumerate}
    \vspace{4pt}
    \item[P1.]
    ...
    \item[P2.]
      ...
      \begin{equation} \label{eq-ilmp}
        \begin{aligned}
           ...
           X_i^{r_{i-1}} \cdot Y_i^{r_i} &= A_i \qquad (\forall \, i \in [2..n-2]) \\
    ...
    \vspace{4pt}
    \item[V2.]
      Accept if the equation~(\ref{eq-ilmp}) holds.

  \end{enumerate}
  }
  \caption{an interactive proof for ILMP. The prover $P$ knows $x_1, \ldots x_n$
    and $y_1, \ldots, y_n$.}
\end{figure}
...
\begin{equation} \label{eq-ilmp-findr}
  r_i - \theta_i = (-1)^{(n-i-1)} \gamma \prod_{j=i+1}^n \frac{y_j}{x_j}
\end{equation}
...
$M^*$ such that
\[
  \Delta(q) = |\Pr[D(x, \brackets{P,V}(x))=1] - \Pr[D(x, M^*(x)|
\]
...
non-negligible. There exists an adversary $\advA$ whose advantage in the DDH
game outlined in section~\ref{sec-ddh} is non-negligible. We omit the details of
the proof here, but this can be shown via a concrete reduction from $\advA$ to
$\advD$
