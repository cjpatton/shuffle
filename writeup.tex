...
\subsection{The basic protocol and its security} \label{section:ilmp}
...
\begin{equation} \label{eq-ilmp-findr}
  r_i - \theta_i = (-1)^{(n-i-1)} \gamma \prod_{j=i+1}^n {y_j} \cdot (q - x_j)
\end{equation}
...
\subsection{Simple $n$-shuffle} \label{sec-simple-n-shuffle}
Let $n \in \N$, $x_1, \ldots, x_n \in [q-1]$, and $y_1, \ldots y_n \in [q-1]$.
...
\begin{figure}[h]
  ...
  \begin{enumerate}
    ...
    \item[V1.] Let $t \getsr [q-1]$, $U = D^t$, and $W = C^t$. Let $\hat{X}_i =
      X_i / U$ and $\hat{Y}_i = Y_i / W$ for every $i \in [n]$. Send $t$ to $P$.

    \vspace{4pt}
    \item[P1.] Let $U = g^{dt}$ and $W = g^{ct}$. Let $\hat{X}_i = X_i / U$ and
      $\hat{Y}_i = Y_i / W$ for every $i \in [n]$. Run
      $\textsc{Silmp}_{\brackets{P,V}}\big( (\hat{X}_1, \dots, \hat{X}_n), (C,
      \dots, C) \big)$.

    \item[P2.]
      Run $\textsc{Silmp}_{\brackets{P,V}}\big( (\hat{Y}_1, \dots, \hat{Y}_n),
      (D, \dots, D) \big)$.
    \vspace{4pt}
  \end{enumerate}
  }
