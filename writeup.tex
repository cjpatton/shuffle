...
$j$ inclusive.
Let $y \gets \brackets{P, V}(x)$ denote the execution of interactive Turing machines $P$
and $V$ on common input $x$. When the protocol finishes, the output of $V$ is
assigned to $y$.
A function $\Delta(n)$ is negligible if for all positive polynomials
...
the adversary $\advD$ plays is defined as follows: let $a, b, r \sample
[1..q-1]$ and let $A = g^a$, $B = g^b$, $Y_0 = g^r$, and $Y_1 = g^{ab}$. Choose
a random bit $b \sample \bits$. On input $(A, B, Y_b)$, the adversary outputs a
...
formal languages with interactive proof systems denoted $\class{IP}$. A language $L$ has an
...
$x \not\in L$, $\Pr[\brackets{P, V}(x) = 1] \le 1/2$.
...
consider polynomial-time adversaries.} and let $x \sample L$. Let $y_1 \sample
\brackets{P, V^*}(x)$ and $y_0 \sample M^*(x)$ choose a bit $b \sample \bits$.
